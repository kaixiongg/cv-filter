...
def gaussian_filter(image, sigma):
    ...
    kernel_size = min(kernel_size, min(H,W)//2)     # make sure kernel isn't too big. 
...
def visualize_maxima(image, maxima, min_sigma, k, file_path=None):
    ...
    for maximum in maxima:
        ...
        circ = plt.Circle((y, x), radius, color='r', fill=False)
        ax.add_patch(circ)
...
def main():
    ...
    # visualize maxima 
    maxima = find_maxima(DoG_small, k_xy=sigma_1)
    ...
    # visualize maxima 
    maxima = find_maxima(DoG_large, k_xy=sigma_1)
